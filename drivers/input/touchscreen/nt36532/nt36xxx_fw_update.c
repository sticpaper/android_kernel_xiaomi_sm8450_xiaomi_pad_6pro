...
int32_t nvt_update_firmware(char *firmware_name)
{
	...
	if(0 == strcmp(firmware_name, BOOT_UPDATE_FIRMWARE_NAME)) {
		if(ts->lcd_id_value) {
			firmware_name = BOOT_UPDATE_FIRMWARE_NAME_FIRST;
		} else {
			firmware_name = BOOT_UPDATE_FIRMWARE_NAME_SECOND;
		}
	} else if(0 == strcmp(firmware_name, MP_UPDATE_FIRMWARE_NAME)) {
		if(ts->lcd_id_value) {
			firmware_name = MP_UPDATE_FIRMWARE_NAME_FIRST;
		} else {
			firmware_name = MP_UPDATE_FIRMWARE_NAME_SECOND;
		}
	}
